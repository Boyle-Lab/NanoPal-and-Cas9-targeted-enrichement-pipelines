...
mv ${FC}.workspace.${MEI}.yb ${FC}.workspace.old
mkdir ${FC}.workspace.${MEI}.yb
cd ${FC}.workspace.${MEI}.yb
...
#######MEI
blastn -evalue 0.001 -task blastn -gapopen 4 -query ${AluYb8} -subject blastn/${t}.fasta -outfmt "7 qacc sacc evalue qstart qend sstart send" > blastn/${t}.blastn.txt
...
#######MEI
awk 'BEGIN{flag=0}{if($2~"'$a'"&&$5>227&&($6<100||$7<100)&&($6<$7)) flag+=1} END{if(flag>=1) print "1";else print "0"}' blastn_refine.all.txt >> palmer.5.plus.txt
awk 'BEGIN{flag=0}{if($2~"'$a'"&&$5>227&&($6<100||$7<100)&&($6>$7)) flag+=1} END{if(flag>=1) print "1";else print "0"}' blastn_refine.all.txt >> palmer.5.minus.txt
awk 'BEGIN{flag=0}{if($2~"'$a'"&&$5>227&&($6>'$b_fix'||$7>'$b_fix')&&($6<$7)) flag+=1} END{if(flag>=1) print "1";else print "0"}' blastn_refine.all.txt >> palmer.3.plus.txt
awk 'BEGIN{flag=0}{if($2~"'$a'"&&$5>227&&($6>'$b_fix'||$7>'$b_fix')&&($6>$7)) flag+=1} END{if(flag>=1) print "1";else print "0"}' blastn_refine.all.txt >> palmer.3.minus.txt
...
less capture.loci.ref.all | grep AluYb |  awk '{print $1,$2,$2+1,$3,$4,$5,"Nanopore"}' > capture.loci.r.AluYb8
less capture.loci.ref.all | grep -v AluYb | grep AluYa  |  awk '{print $1,$2,$2+1,$3,$4,$5,"Nanopore"}' > capture.loci.r.AluYa5
