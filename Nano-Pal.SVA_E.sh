...
mv ${FC}.workspace.${MEI}.svae ${FC}.workspace.old
mkdir ${FC}.workspace.${MEI}.svae
cd ${FC}.workspace.${MEI}.svae
...
#######MEI
blastn -evalue 0.001 -task blastn -gapopen 4 -query ${SVA_E} -subject blastn/${t}.fasta -outfmt "7 qacc sacc evalue qstart qend sstart send" > blastn/${t}.blastn.txt
...
cat blastn/${t}.blastn.txt | grep -v "#"  | awk 'BEGIN{flag=0}{if($5>930&&($6<100||$7<100)&&($6<$7)) flag+=1} END{if(flag>=1) print "1";else print "0"}' >> 5.plus.txt 
cat blastn/${t}.blastn.txt | grep -v "#"  | awk 'BEGIN{flag=0}{if($5>930&&($6<100||$7<100)&&($6>$7)) flag+=1} END{if(flag>=1) print "1";else print "0"}' >> 5.minus.txt 
cat blastn/${t}.blastn.txt | grep -v "#"  | awk 'BEGIN{flag=0}{if($5>930&&($6>'$len_fix'||$7>'$len_fix')&&($6<$7)) flag+=1} END{if(flag>=1) print "1";else print "0"}' >> 3.plus.txt 
cat blastn/${t}.blastn.txt | grep -v "#"  | awk 'BEGIN{flag=0}{if($5>930&&($6>'$len_fix'||$7>'$len_fix')&&($6>$7)) flag+=1} END{if(flag>=1) print "1";else print "0"}' >> 3.minus.txt 
...
#######MEI
awk 'BEGIN{flag=0}{if($2~"'$a'"&&$5>930&&($6<100||$7<100)&&($6<$7)) flag+=1} END{if(flag>=1) print "1";else print "0"}' blastn_refine.all.txt >> palmer.5.plus.txt
awk 'BEGIN{flag=0}{if($2~"'$a'"&&$5>930&&($6<100||$7<100)&&($6>$7)) flag+=1} END{if(flag>=1) print "1";else print "0"}' blastn_refine.all.txt >> palmer.5.minus.txt
awk 'BEGIN{flag=0}{if($2~"'$a'"&&$5>930&&($6>'$b_fix'||$7>'$b_fix')&&($6<$7)) flag+=1} END{if(flag>=1) print "1";else print "0"}' blastn_refine.all.txt >> palmer.3.plus.txt
awk 'BEGIN{flag=0}{if($2~"'$a'"&&$5>930&&($6>'$b_fix'||$7>'$b_fix')&&($6>$7)) flag+=1} END{if(flag>=1) print "1";else print "0"}' blastn_refine.all.txt >> palmer.3.minus.txt
...
less capture.loci.ref.all | grep SVA_E |  awk '{print $1,$2,$2+1,$3,$4,$5,"Nanopore"}' > capture.loci.r.SVA_E
less capture.loci.ref.all | grep -v SVA_E | grep SVA_F  |  awk '{print $1,$2,$2+1,$3,$4,$5,"Nanopore"}' > capture.loci.r.SVA_F
